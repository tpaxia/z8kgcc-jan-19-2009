...
(define_attr "type"
 "cbranch,jump,jump_ind,arith,arith3,arith3b,dyn_shift,other,load,load_si,store,move,fmove,smpy,dmpy,return,pload,pstore,pcload,pcload_si,rte,sfunc,call,fp,fdiv,dfp_arith,dfp_cmp,dfp_conv,dfdiv,nil,gp_fpul"
  (const_string "other"))
  ...
  if (TARGET_SH4)
    {
      ...
      emit_move_insn (operands[3], gen_rtx (SYMBOL_REF, SImode, \"__udivsi3\"));
  ...
  if (TARGET_SH4)
    {
      emit_move_insn (gen_rtx (REG, SImode, 4), operands[1]);
      emit_move_insn (gen_rtx (REG, SImode, 5), operands[2]);
      emit_move_insn (operands[3], gen_rtx (SYMBOL_REF, SImode, \"__sdivsi3\"));
      if (TARGET_FPU_SINGLE)
  ...
  "
{
  ...
  if (!TARGET_SH2)
    {
      rtx insns = gen_mulsi3_call (operands[0], operands[1], operands[2]);
...
(define_insn "push_fpul"
  [(set (mem:SF (pre_dec:SI (reg:SI 15))) (reg:SI 22))]
...
(define_insn "pop_fpul"
  [(set (reg:SI 22) (mem:SF (post_inc:SI (reg:SI 15))))]
  "TARGET_SH3E | TARGET_SH4"
  "lds.l	@r15+,fpul"
  [(set_attr "type" "load")
   (set_attr "hit_stack" "yes")])
...
;; All alterntives of movdf_i4 are split for ! TARGET_FMOVD.
...
(define_insn "movdf_i4"
  [(set (match_operand:DF 0 "general_movdst_operand" "=d,r,d,d,m,r,r,m,!??r,!???d")
   ...
   (use (match_operand:PSI 2 "register_operand" "c,c,c,c,c,c,c,c,c,c"))
  ...
  [(set_attr_alternative "length"
     [(if_then_else (eq_attr "endian" "big") (const_int 2) (const_int 4))
      (const_int 4)
      (if_then_else (eq_attr "endian" "big") (const_int 4) (const_int 6))
      (if_then_else (eq_attr "endian" "big") (const_int 2) (const_int 6))
      (if_then_else (eq_attr "endian" "big") (const_int 2) (const_int 6))
      (const_int 4)
...
(define_split
  [(set (match_operand:SF 0 "register_operand" "")
   ...
   (use (match_operand:PSI 2 "register_operand" ""))
   (clobber (match_scratch:SI 3 "X"))]
...
;; Little-endian post-reload splits
(define_split
  [(set (match_operand:DF 0 "register_operand" "")
	(match_operand:DF 1 "register_operand" ""))
   (use (match_operand:PSI 2 "register_operand" "c"))
  ...
  if (GET_CODE (addr) != POST_INC)
    {
      ...
      XEXP (mem2, 0) = addr = gen_rtx (POST_INC, SImode, addr);
    }
  addr = XEXP (addr, 0);
  insn = emit_insn (gen_movsf_ie (gen_rtx (REG, SFmode,
					   regno + !! TARGET_LITTLE_ENDIAN),
...
(define_insn "movsf_ie"
  [(set (match_operand:SF 0 "general_movdst_operand"
   ...
   (use (match_operand:PSI 2 "register_operand" "c,c,c,c,c,c,c,c,c,c,c,c,c,c,c,c,c"))
...
;; ??? reload currently cannot handle 3-operand instructions correctly when
;; there is a matching constraint and all three operands need the same
;; secondary reload register.
;; There is also the problem that reload does not consider secondary
;; reloads more costly than ordinary reloads.  Thus, when you have
;;  [(set (match_operand:SF 0 "arith_reg_operand" "=y,?f")
;;	(plus:SF (match_operand:SF 1 "arith_reg_operand" "%0,0")
;;		 (match_operand:SF 2 "arith_reg_operand" "r,f")))
;;   (clobber (match_scratch:SF 3 "=&f,X"))
;;   (clobber (match_scratch:SF 4 "=f,X"))]
;; the '?' for the second alternative is a lie, but necessary to avoid
;; that reload picks the second alternative and reloads both arguments,
;; resulting in a FPUL_REGS RELOAD_FOR_OTHER_ADDRESS which conflicts with
;; a FPUL_REGS RELOAD_FOR_INPUT_ADDRESS.
;; Therefore, we do not even pretend that we have single-precision
;; binary arithmetic operations for the SH4 target.  Rather, we allow
;; all the extra conversions to be generated, and remove them as we see fit.
...
(define_insn "fpu_switch"
  [(set (match_operand:PSI 0 "register_operand" "c,c,c,c,r")
	(match_operand:PSI 1 "general_movsrc_operand" "c,>,m,r,m"))]
  ""
  "@
...
(define_insn "*macsf3"
  [(set (match_operand:SF 0 "arith_reg_operand" "=f")
   ...
   (use (match_operand:PSI 4 "register_operand" "c"))]
  "TARGET_SH3E && ! TARGET_SH4"
  "fmac	fr0,%2,%0"
  [(set_attr "type" "fp")])
...
(define_insn "floatsisf2_i4"
  [(set (match_operand:SF 0 "arith_reg_operand" "=f")
	(float:SF (reg:SI 22)))
   (use (match_operand:PSI 1 "register_operand" "c"))]
  "TARGET_SH3E"
  "float	fpul,%0"
  [(set_attr "type" "fp")])
