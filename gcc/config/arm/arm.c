  ...
  for (i = sizeof (arm_select) / sizeof (arm_select[0]); i--;)
    {
	  ...
	  struct processors * sel;
		...
		if (flags != 0)
		  {
		    if (flags != sel->flags)
		      error ("switch %s%s overridden by another switch", ptr->string, sel->name );
		  }
		else
		  {
		    flags = sel->flags;
     ...
     int max_count;
{
  int count = 0;
  rtx found_barrier = 0;
  rtx label; /* CYGNUS LOCAL nickc */

  while (from && count < max_count)
    {
      if (GET_CODE (from) == BARRIER)
	return from; /* CYGNUS LOCAL nickc */
  ...
  if (live_regs)
    {
	  ...
	  strcat (instr, "%|");
          ...
          strcat (instr, ", ");
	  strcat (instr, "%|");
	  strcat (instr, TARGET_THUMB_INTERWORK || (! really_return) ? reg_names[14] : reg_names[15] );
        }
      else
	{
	  strcat (instr, "%|");
     ...
     int frame_size;
{
  ...
  /* The check for stack limit violation is made at the end of the
     function entry sequence, by which time ip/r12 is available as a
     work register.  */
...
arm_expand_prologue ()
{
  ...
  /* If we are profiling, make sure no instructions are scheduled
     before the call to mcount.  Similarly do not allow instructions
