     ...
     enum machine_mode mode;
{
  ...
  if (mode == SImode || mode == SFmode || mode == PSImode) {
    ...
    if (mode == PSImode && r_da_operand (operands[0], mode)) {
#if 0
      printf("Bingo:\n");
      printf("\tr_operand: %d\n", r_operand (operands[1], mode));
      printf("\trx1: %d\n", r_ir_da_x_ba_bx_operand (operands[1], mode));
      printf("\trx2: %d\n", r_im_ir_da_x_operand (operands[1], mode));
      printf("\tda: %d\n", da_operand (operands[1], mode));
      printf("\tba: %d\n", ba_operand (operands[1], mode));
      printf("\tbx: %d\n", bx_operand (operands[1], mode));
      printf("\tx: %d\n", x_operand (operands[1], mode));
#endif
      if (warn_cpg > 1)
        {
          printf("\n*** This is debug output enabled by multiple '-Wcpg' parameters:\n");
          printf("mode: %d\n", mode);
          printf("r_da: %d, imm: %d\n",
                 r_da_operand (operands[0], mode),
                 immediate_operand (operands[1], mode));
