...
;;   Copyright (C) 1992, 1993, 1994, 2005, 2007, 2009 Free Software Foundation, Inc.
...
(define_attr "type" "branch,foo,def,djnz,invisible,return,pushqi" 
  (const_string "def"))
...
(define_attr "length" ""
  (cond [(eq_attr "type" "branch")
	 ...
	 (eq_attr "type" "pushqi") (const_int 2)   ;; CPG
...
(define_insn "movhi_matcher"
  ...
  [(set_attr "cond" "trashcc,notrashcc,notrashcc,notrashcc,notrashcc,notrashcc,notrashcc,notrashcc,notrashcc")
   (set_attr "length" "4,6,6,6,2,6,6,6,2")
  ])
...
(define_insn ""
  [(set (match_operand:PSI 0 "r_operand" "=r")
       (plus:PSI (match_operand:PSI 1 "symbol_ref" "i")
		 (match_operand:PSI 2 "r_operand" "v")))]
  ""
  "*
{
	if (warn_cpg) warning (\"cpg machine description change #1 is being used, program may not work (disable warning with '-Wno-cpg')\");
	return \"lda	%S0,%A1(%I2) !i8 CPG\";
}")

;(insn 26 24 31 (set (reg:PSI 24)
;        (mem:SI (symbol_ref:PSI ("ix")))) -1 (nil)
;    (nil))
;; CPG2
(define_insn ""
  [(set (match_operand:PSI 0 "r_operand" "=r")
        (mem:SI (match_operand:PSI 1 "symbol_ref" "")))]
  ""
  "*
{
	if (warn_cpg) warning (\"cpg machine description change #2 is being used, program may not work (disable warning with '-Wno-cpg')\");
	return \"ldl	%S0,%A1	!xxxx CPG\";
}")

;(insn/i 10 9 11 (set (reg:PSI 24)
;        (mem/s:SI (plus:PSI (reg/v:PSI 23)
;                (const_int 1236)))) -1 (insn_list 8 (nil))
;    (nil))
;; CPG3
(define_insn ""
  [(set (match_operand:PSI 0 "r_operand" "=r")
	(mem:SI (plus: PSI
	         (match_operand:PSI 1 "r_operand" "r")
		 (match_operand:HI 2 "immediate_operand" "i"))))]
  ""
  "*
{
	if (warn_cpg) warning (\"cpg machine description change #3 is being used, program may not work (disable warning with '-Wno-cpg')\");
	return \"ldl	%S0,%S1(%H2)	!yyyy CPG\";
}"
  [(set_attr "cond" "logcc")])


;(insn/i 118 117 119 (set (reg:PSI 49)
;        (reg:SI 48)) -1 (insn_list 117 (nil))
;    (nil))
;; CPG4
(define_insn ""
  [(set (match_operand:PSI 0 "r_operand" "=r")
        (match_operand:SI 1 "r_operand" "r"))]
  ""
  "*
{
	if (warn_cpg) warning (\"cpg machine description change #4 is being used, program may not work (disable warning with '-Wno-cpg')\");
	return \"ldl	%S0,%S1	!zzzz CPG\";
}")
