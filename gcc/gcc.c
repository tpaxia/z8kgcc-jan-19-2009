...
%{!fsyntax-only: \
 %{!c:%{!M:%{!MM:%{!E:%{!S:collect2 %l %X %{o*} %{A} %{d} %{e*} %{m} %{N} %{n} \
			%{r} %{s} %{t} %{u*} %{x} %{z} %{Z}\
...
execute ()
{
  ...
  /* We'll look for ld if we can't find collect2 anywhere. */
  if (!string && strcmp (commands[0].prog, "collect2") == 0)
    {
      commands[0].prog = "ld";
      string = find_a_file (&exec_prefixes, commands[0].prog, X_OK);
    }
...
#if defined(__CYGWIN__)  /* hack */
  for (temp = gcc_exec_prefix; *temp; temp++)
    if (*temp == '\\') *temp = '/';
#endif
  /* END CYGNUS LOCAL -- meissner/relative pathnames */
