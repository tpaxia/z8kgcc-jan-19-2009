...
/* Compute the predecessors and successors for each block.  Return nonzero
   if edges were split (which indicates to our caller that find_basic_blocks
   must be called again to find blocks created by edge splitting).  */
int
compute_preds_succs (s_preds, s_succs, num_preds, num_succs, split_edges)
     ...
     int split_edges;
{
  int bb, changed, clear_local_bb_vars = 0;
  ...
  /* Assume that we do not need to rerun find_basic_blocks by default.  */
  changed = 0;
