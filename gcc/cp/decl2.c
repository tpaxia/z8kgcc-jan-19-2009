...
/* CYGNUS LOCAL: ECC Linker */
/* Nonzero means output .vtable_{entry,inherit} for use in doing vtable gc.  */

int flag_vtable_gc = 0;

/* END CYGNUS LOCAL: ECC Linker */
     ...
     tree prev, vars;
{
  if (write_virtuals >= 0
      ...
      if (flag_vtable_gc)
	{
	   ...
	   * straightforward how to get this during multiple inheritence.
	   */
	  ...
	  bases = DECL_CONTEXT (vars);
	  bases = TYPE_BINFO_BASETYPES (bases);
	  ...
	  if (bases == NULL)
	  ...
	  else
	    {
	      ...
	      /* Assume we want the left-most base class.  */
	      parent = TREE_VEC_ELT (bases, 0);

	      /* Look for the vtable in the base classes from the right.  */
	      for (i = TREE_VEC_LENGTH (bases); --i > 0; )
		{
		  tree b = TREE_VEC_ELT (bases, i);
		  if (BINFO_VTABLE (b) == vars)
		    {
		      parent = TYPE_BINFO (TREE_TYPE (b));
		      break;
		    }
		}

	      parent = BINFO_VTABLE (parent);
...
static void
finish_objects (method_type, initp, started)
     int method_type, initp, started;
{
  char *fnname;
