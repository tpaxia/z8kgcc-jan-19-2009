...
int
z8k_get_breaks (bfd_vma loc,
                bfd_vma breaks[],
                int (*getmem) (bfd_vma memaddr,
                               bfd_byte * myaddr,
                               unsigned int len,
                               disassemble_info * info),
                unsigned long (*rrfunc)(int regnum),
                int is_segmented,
                int *condjmp);
                ...
                int *condjmp)
{
  ...
  int retoff;
  unsigned long tmp;
  unsigned char stackbytes[4];
  instr_data_s instr_data;

  *condjmp = 0;
  breaks[0] = breaks[1] = 0;
  memset (&info, 0, sizeof(info));
  info.read_memory_func = getmem;
  /* info.memory_err_func = what?? XXX FIXME */

  info.private_data = (PTR) &instr_data;
  instr_data.max_fetched = 0;
  instr_data.insn_start = loc;
  ...
  if (instr_data.tabl_index >= 0)
    {
      unpack_instr (&instr_data, is_segmented, &info);   /* Sets seg_length and other things.  */

      /* Instruction has been identified.  Knowing its length we can set break[0].  */
      breaks[0] = breaks[1] = loc + z8k_table[instr_data.tabl_index].length + seg_length;
#if 0
      printf("z8k_get_breaks: next instruction at %08lx\n", breaks[0]);
#endif

      tabl_datum = z8k_table[instr_data.tabl_index].arg_info[0];
      datum_class = tabl_datum & CLASS_MASK;
      datum_value = tabl_datum & ~CLASS_MASK;

      /* If is is a branch instruction we have to find out the target address.  */
      switch(z8k_table[instr_data.tabl_index].opcode)
        {
        case OPC_iret:  /* XXX FIXME: Untested.  */
          retoff = 4;
        case OPC_ret:
          tmp = rrfunc(22 /*SP_REGNUM*/);    /* Get stack pointer.  */
          if (is_segmented)
            {
              getmem(tmp + retoff, stackbytes, 4, NULL);
              breaks[1] = (stackbytes[0] << 24)
                | (stackbytes[1] << 16)
                | (stackbytes[2] << 8)
                | stackbytes[3];
            }
            ...
            {
              getmem(tmp + retoff, stackbytes, 2, NULL);
              breaks[1] = (stackbytes[0] << 8) | stackbytes[1];
            }
          return 0;

        case OPC_jp:
          tabl_datum = z8k_table[instr_data.tabl_index].arg_info[1];
          datum_class = tabl_datum & CLASS_MASK;
          datum_value = tabl_datum & ~CLASS_MASK;
#if 0
          printf("z8k_get_breaks: JP (datum_class: 0x%x)\n",datum_class );
#endif
          *condjmp = 1;
          /* Fall thru.  */
        case OPC_call:
          switch(datum_class)
            {
            case CLASS_IR:
              tmp = rrfunc(instr_data.arg_reg[datum_value]);
              if (is_segmented)
                {
                  tmp = (tmp << 16) | rrfunc(instr_data.arg_reg[datum_value] + 1);
                }
              breaks[1] = tmp;
#if 0
              printf("z8k_get_breaks: CALL/JP indirect register (%d -> %08x)\n",
                     instr_data.arg_reg[datum_value],
                     breaks[1]);
#endif
              break;
            case CLASS_DA:
              breaks[1] = instr_data.address;
              break;
            case CLASS_X:
              tmp = rrfunc(instr_data.arg_reg[datum_value]);
              breaks[1] = instr_data.address + tmp;
#if 0
              printf("z8k_get_breaks: CALL/JP indexed, break at %08x\n", breaks[1]);
#endif
              break;
            }
          return 0;
        ...
        case OPC_dbjnz:
        case OPC_calr:
          breaks[1] = instr_data.displacement;
          return 0;
        }
      return 0;    /* Success.  */
    }
